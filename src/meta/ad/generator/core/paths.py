...
class Paths:
    ...
    def prompts_output(
        ...
        Example:
            paths.prompts_output()
            # → config/ad/generator/prompts/moprobo/taboola/2026-01-26/
        """
        cust = customer.lower().replace("-", "_") if customer else self.customer
        plat = platform.lower().replace("-", "_") if platform else self.platform
        d = date or self.date

        return self.config_dir / "ad" / "generator" / "prompts" / cust / plat / d
    ...
    def prompts_file(
        ...
        """
        return (
            self.prompts_output(customer, platform, date)
            / filename
        )
