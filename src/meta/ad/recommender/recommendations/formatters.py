...
# Import evidence builder at module level
from .evidence_builder import EvidenceBuilder, EvidenceFormatter
...
class EnhancedRecommendation:
    ...
    # Internal evidence attachment (not in __init__)
    _evidence: Optional[Dict[str, Any]] = field(default=None, init=False)
...
class RecommendationOutputFormatter:
    ...
    def _format_markdown_summary(
        self, output: EnhancedRecommendationOutput, metric_label: str
    ) -> list:
        """Format the summary section for markdown output."""
        lines = []
    ...
    def _format_markdown_recommendation(
        self, rec: EnhancedRecommendation, i: int, metric_label: str
    ) -> list:
        ...
        lines.append(f"**Recommended:** {rec.recommended_value.get('value')}")

        # Display opportunity_size if available
        if hasattr(rec, "_evidence") and rec._evidence.opportunity_size:
            lines.append(
                f"**Opportunity Size:** {self.evidence_formatter.format_opportunity_size(rec._evidence.opportunity_size)}"
            )
        else:
            lines.append(
        ...
        if isinstance(confidence, str):
            confidence_str = confidence.title()
        else:
        ...
        if hasattr(rec, "_evidence") and rec._evidence.has_strong_evidence:
            lines.append("")
            lines.append("**Evidence:**")
            lines.append(
                self.evidence_formatter.format_evidence_summary(rec._evidence)
            )
        ...
        if rec.risk_factors:
            ...
            for risk_factor in rec.risk_factors:
                ...
                lines.append(f"  - Mitigation: {risk_factor.mitigation}")
            lines.append("")
    ...
    def to_markdown(self, output: EnhancedRecommendationOutput) -> str:
        ...
        # Summary
        lines.extend(self._format_markdown_summary(output, metric_label))
        ...
        for group in output.groups:
            ...
            for i, rec in enumerate(group.recommendations, 1):
                lines.extend(
                    self._format_markdown_recommendation(rec, i, metric_label)
                )
        ...
        if quick_wins:
            ...
            for group in quick_wins:
                for rec in group.recommendations[:3]:
                    ...
                    # Use compact opportunity_size if available
                    if (
                        hasattr(rec, "_evidence")
                        and rec._evidence.opportunity_size
                    ):
                        impact_text = self.evidence_formatter.format_compact_opportunity_size(
                            rec._evidence.opportunity_size
                        )
                    else:
                        impact_text = f"Impact: {rec.recommended_value.get('expected_lift')}"
...
def convert_legacy_output(
    ...
    evidence_builder = EvidenceBuilder(target_col=target_metric)
    ...
    if quick_wins:
        ...
        for rec in quick_wins[:5]:  # Top 5
            ...
            # Build evidence for this recommendation
            evidence = evidence_builder.build_evidence(
                recommendation=rec,
                validated_pattern=None,  # Not available in legacy output
                counterfactual=None,  # Not available in legacy output
                current_metric_value=current_metric_val,
            )
            ...
            enhanced = EnhancedRecommendation(
                ...
                statistical_significance=(
                    evidence.statistical.to_dict()
                    if evidence.statistical
                    else {}
                ),
                cross_customer_validation=(
                    evidence.cross_customer.to_dict()
                    if evidence.cross_customer
                    else None
                ),
                top_performer_analysis=(
                    evidence.top_performer.to_dict()
            ...
            # Attach full evidence object for formatters (internal use)
            enhanced._evidence = evidence  # pylint: disable=protected-access
